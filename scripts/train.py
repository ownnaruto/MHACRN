...
if __name__ == "__main__":
    ...
    parser.add_argument('-l', "--lr", type=float, default=0.004) 
    parser.add_argument('-ms', "--milestones", nargs='*', type=int, default=[55]) 
    parser.add_argument('-ld', "--lr_decay_rate", type=float, default=0.05) 
    
    parser.add_argument('-dpl', "--decoder_patch_len", type=int, default=2)
    parser.add_argument('-sp', "--shared_pred", action='store_true', default=True) 
    parser.add_argument('-sep', "--seperate_pred", action='store_true', default=True)
    parser.add_argument('-rd', '--rnn_dim', type=int, default=96)
    parser.add_argument('-ded', '--decoder_embed_dim', type=int, default=32)
    parser.add_argument('-ted', '--time_embed_dim', type=int, default=8)
    parser.add_argument('-nd', '--node_dim', type=int, default=20)
    parser.add_argument('-sed', '--st_embed_dim', type=int, default=20)
    ...
    # save_path = f"../saved_models/{model_name}/lr{cfg.get('lr')}_bs{cfg.get('batch_size')}"
    ...
    lr_type = cfg.get('scheduler', 'multi')
    if lr_type == 'multi':
        ...
        scheduler = torch.optim.lr_scheduler.ExponentialLR(
            optimizer=optimizer,
            gamma=cfg.get('lr_decay_rate', 0.9),
            verbose=True
        )
    elif lr_type == 'step':
        scheduler = torch.optim.lr_scheduler.StepLR(
            ...
            gamma=cfg.get('lr_decay_rate', 0.7)
        )
    # ----------------------------- set model runner ----------------------------- #
    ...
    pred_save_path = f"../save_prediction/{model_name}"
    if not os.path.exists(pred_save_path):
        os.makedirs(pred_save_path) 
    runner.test_model(model, testset_loader, save_pred=pred_save_path)
    log.close()
